...
export function insertImage(editor: Editor | null): boolean {
  ...
  try {
    const uploadExt = editor.extensionManager.extensions.find(
    ...
    return editor
      ...
      .insertContent({
        ...
        attrs: {
          accept:
            uploadExt?.options?.accept ??
            "image/*,video/*,application/pdf,.pdf,.mp4,.webm,.mov",
          limit: uploadExt?.options?.limit ?? 3,
          maxSize: uploadExt?.options?.maxSize ?? 0,
        },
      })
      .run()
  } catch {
